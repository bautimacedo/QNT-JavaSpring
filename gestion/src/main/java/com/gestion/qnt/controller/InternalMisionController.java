...
public class InternalMisionController {
    ...
    public ResponseEntity<Map<String, Object>> receiveVueloLog(
        ...
        // CAM ATERRIZAJE: la completion de misión y stats la maneja DroneVueloEventListener
        // vía MQTT (DroneVueloEvent). Este endpoint solo persiste el log de vuelo.
    ...
    public ResponseEntity<?> botMisiones(
        ...
        List<Map<String, Object>> result = misionRepository.findAllWithDetails().stream()
                .filter(m -> m.getEstado() == EstadoMision.PLANIFICADA && m.getDron() != null)
                .map(m -> {
                    ...
                    dto.put("site",        m.getDron().getYacimiento() != null ? m.getDron().getYacimiento().name() : null);
    ...
    public ResponseEntity<Map<String, Object>> botLanzarMision(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
                    .body(Map.of("error", "No se pudo lanzar la misión: " + e.getMessage()));
        }
    ...
    private boolean esDroneVolandoMqtt(Dron dron) {
        if (dron.getDock() != null && dron.getDock().getNumeroSerie() != null) {
            ...
            if (dockSnap != null) return Boolean.FALSE.equals(dockSnap.confirmedDroneEnDock);
        }
